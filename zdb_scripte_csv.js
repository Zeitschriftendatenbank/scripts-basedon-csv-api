...
function csvBatchLoeschen(){
	var csv = new CSV();
	
	

	csv.__csvLoeschen = function()
		{
			application.activeWindow.command("show d", false);
			...
			if (application.activeWindow.status != "ERROR") {
				...
				var hitLines = csv.__csvBatchCSVToLoeschenSearch(csv.eigene_bibliothek,lastLineNumber);
				...
				for(var i = 0; i < hitLines.length;i++){
					...
					while(test != true && count < 10){
						...
						test = csv.__csvBatchCSVToLoeschenLineUpAndTest(currentField);
						count++;
					...
					// replace x?? with l; {0,2} : tero or two letters after the x
	...
	csv.__csvLoeschenFreiberg = function()
		{
			application.activeWindow.command("show d", false);
			...
			if (application.activeWindow.status != "ERROR") {
				...
				// search returns array of line counts
				var hitLines = csv.__csvBatchCSVToLoeschenSearch('954000-3',lastLineNumber);


				if(hitLines.length == 0)
				{
					return csv.__csvSaveBuffer(false,"String '954000-3' kann nicht gefunden werden.\n");
				}
				else if(hitLines.length > 1)
				{
					return csv.__csvSaveBuffer(false,"Mehr als ein Exemplar.\n");
				}
	...
	csv.__csvLoeschenAlteFachgruppen = function()
		{
			application.activeWindow.command("show d", false);
			...
			if (application.activeWindow.status != "ERROR") 
			{
				...
				var feld5080 = application.activeWindow.title.findTag("5080",0,true,true,true);
				
				if(feld5080 == '')
				{
					return csv.__csvSaveBuffer(false,"Feld '5080' ist nicht vorhanden.\n");
				}

				// delete line
				application.activeWindow.title.deleteSelection();
				
				var newFeld5080 = feld5080.replace(/\%z{0,1}\d\d\d/g,'');
				
				application.activeWindow.title.endOfBuffer(false);
				
				application.activeWindow.title.insertText(newFeld5080);
				
				csv.__csvLOG("\tErsetzt: "+ feld5080 + " gegen " + newFeld5080 + " in Zeile " + application.activeWindow.title.currentLineNumber);

				
				
			//	save buffer
				return csv.__csvSaveBuffer(true,newFeld5080);
	...
	csv.__csvBatchCSVToLoeschenSearch = function (term,lastline)
		{
			var lines = new Array();
	...
	csv.__csvBatchCSVToLoeschenLineUpAndTest = function (currentField)
		{
			// go one line up
			application.activeWindow.title.lineUp(1,false);
	...
	//csv.__csvSetProperties(csv.__csvLoeschen,["","ZDB-ID"],'ZDB-ID','zdb',true,"ZDB_LOG.txt");
	//csv.__csvSetProperties(csv.__csvLoeschenFreiberg,["","ZDB-ID"],'ZDB-ID','zdb',false,"ZDB_LOG_Freiberg.txt");
	csv.__csvSetProperties(csv.__csvLoeschenAlteFachgruppen,["","ZDB-ID"],'ZDB-ID','zdb',false,"ZDB_LOG.txt");
